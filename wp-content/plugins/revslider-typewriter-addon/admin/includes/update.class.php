...
class rs_typewriter_update {
	...
	private $remote_url_info	= 'http://updates.themepunch.tools/addons/revslider-typewriter-addon/revslider-typewriter-addon.php';
	private $plugin_slug		= 'revslider-typewriter-addon';
	private $plugin_path		= 'revslider-typewriter-addon/revslider-typewriter-addon.php';
	private $version;
	private $plugins;
	private $option;
	...
	protected function _check_updates() {
		...
		$last_check = get_option('rs_typewriter-update-check');


		if($last_check == false){ //first time called
			$last_check = time();
			update_option('rs_typewriter-update-check', $last_check);
		}
		...
		if(time() - $last_check > 172800 || $force_check == true){
			...
			if(isset($data->basic)) {
				update_option('rs_typewriter-update-check', time());
				
				$this->data->checked = time();
				$this->data->basic = $data->basic;
				$this->data->full = $data->full;
					
				//update_option('rs_typewriter-stable-version', $data->full->stable);
				update_option('rs_typewriter-latest-version', $data->full->version);
			}
	...
	public function _retrieve_update_info() {
		...
		$validated = get_option('rs_typewriter-valid', 'false');
	...
	public function _retrieve_version_info($force_check = false) {
		...
		$last_check = get_option('rs_typewriter-update-check-short');
		if($last_check == false){ //first time called
			$last_check = time();
			update_option('rs_typewriter-update-check-short', $last_check);
		}
		...
		if(time() - $last_check > 172800 || $force_check == true){
			...
			update_option('rs_typewriter-update-check-short', time());
			...
			$response = wp_remote_post($this->remote_url, array(
				'user-agent' => 'WordPress/'.$wp_version.'; '.get_bloginfo('url'),
				'body' => array(
					'item' => urlencode('revslider-typewriter-addon'),
					'version' => urlencode($this->version),
					'code' => urlencode($purchase)
				)
			));
			...
			if ( $response_code != 200 || is_wp_error( $version_info ) ) {
				update_option('rs_typewriter-connection', false);
				return false;
			}else{
				update_option('rs_typewriter-connection', true);
			}
			
			/*
			$version_info = json_decode($version_info);
			if(isset($version_info->version)){
				update_option('rs_typewriter-latest-version', $version_info->version);
			}
			
			if(isset($version_info->notices)){
				update_option('rs_typewriter-notices', $version_info->notices);
			}
			
			if(isset($version_info->dashboard)){
				update_option('rs_typewriter-dashboard', $version_info->dashboard);
			}
			
			if(isset($version_info->deactivated) && $version_info->deactivated === true){
				if(get_option('rs_typewriter-valid', 'false') == 'true'){
					//remove validation, add notice
					update_option('rs_typewriter-valid', 'false');
					update_option('rs_typewriter-deact-notice', true);
				}
			}
			*/
		}
		...
		if($force_check == true){ //force that the update will be directly searched
			update_option('rs_typewriter-update-check', '');
		}
