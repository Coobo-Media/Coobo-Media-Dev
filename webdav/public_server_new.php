...
$authBackend = new Sabre\DAV\Auth\Backend\BasicCallBack(function($userName, $password) {
	 ...
	 global $pdo;
	 global $realm;
	 $stmt = $pdo->prepare('SELECT username, digesta1 FROM users WHERE username = ?');
     $stmt->execute(array($userName));
     $result = $stmt->fetchAll();
	 //if no results username is not present
	 if (!count($result)) return false;
	 $digesta1 = $result[0]['digesta1'];
	 $hash = md5($userName . ':' . $realm . ':' . $password);
	
   	 // check the password here, and then just return true or false.
   	 return ($hash == $digesta1 ? true: false);
...
require_once('MyServer/MyHomeCollection.php');

$tree = array( 
	new MyServer\MyHomeCollection($authPlugin,$pdo),
);
